...
function App() {
    ...
    const [isMuted, setIsMuted] = useState(true); // Mic state DEFAULT MUTED (User Request)
    const isMutedRef = useRef(false);
    
    useEffect(() => {
        isMutedRef.current = isMuted;
    }, [isMuted]); // Mic state DEFAULT MUTED
    ...
    const downsampleBuffer = (buffer, sampleRate, outSampleRate) => {
        ...
        if (outSampleRate > sampleRate) {
            return buffer; // Cannot upsample efficiently here
        }
        ...
        if (isConnected && isAuthenticated && socketConnected && micDevices.length > 0 && !hasAutoConnectedRef.current) {
            ...
            const timer = setTimeout(() => {
                ...
                socket.emit('start_audio', {
                    ...
                    muted: isMuted,
                    use_client_audio: true // Always stream from client
                });
            }, 500);
        }
    ...
    useEffect(() => {
        ...
        // Initial Device Setup: Request Permissions & Enumerate
        // We request both Audio and Video permissions immediately to ensure the user is prompted for both.
        // This fixes the issue where only the camera prompt appears.
        const initDevices = async () => {
            try {
                // Force permission request
                console.log("Requesting initial Audio/Video permissions...");
                const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
                
                // Got it! Now stop the streams, we just needed the permission.
                stream.getTracks().forEach(t => t.stop());
                console.log("Permissions granted. Enumerating devices...");
            } catch (err) {
                console.warn("Initial permission request failed/cancelled:", err);
                // Continue anyway, maybe they only denied one, or we can list blindly
            }
            ...
            try {
                ...
                // No 'else' needed here as we forced permission above, so if it's empty, it's really empty.
    ...
    const startMicVisualizer = async (deviceId) => {
        stopMicVisualizer();
        try {
            // Ensure we have a valid, open AudioContext
            if (!audioContextRef.current || audioContextRef.current.state === 'closed') {
                audioContextRef.current = new (window.AudioContext || window.webkitAudioContext)();
            }

            // Browser Security Check for non-localhost
            if (window.location.hostname !== 'localhost' && window.location.protocol === 'http:') {
                console.warn("Microphone access on HTTP (non-localhost) is often blocked by browsers.");
                addMessage({ sender: 'System', text: "Warning: Microphone might be blocked on HTTP. Use Chrome flags or localhost." });
            }

            // IMPORTANT: Resume context (browsers suspend it until user interaction)
            if (audioContextRef.current.state === 'suspended') {
                await audioContextRef.current.resume();
            }

            // If deviceId is null/empty, use bare request to prompt user or get default
            const constraints = deviceId ? { audio: { deviceId: { exact: deviceId } } } : { audio: true };
            const stream = await navigator.mediaDevices.getUserMedia(constraints);
            
            // If usage was generic, update selected ID to what we actually got
            if (!deviceId) {
            ...
            // Log success to help debugging
            console.log("Microphone stream acquired:", stream.id);


            analyserRef.current = audioContextRef.current.createAnalyser();
            analyserRef.current.fftSize = 256; // Changed from 64 to 256
            ...
            const updateMicData = () => {
                ...
                // Simple sum for visualizer "energy"
            ...
            // Setup Audio Streaming (Using AudioWorklet)
            const blob = new Blob([audioWorkletCode], { type: 'application/javascript' });
            const workletUrl = URL.createObjectURL(blob);
            
            // Add module (idempotent-ish, strictly we should handle re-adding but blob url is new each time)
            await audioContextRef.current.audioWorklet.addModule(workletUrl);
            
            const workletNode = new AudioWorkletNode(audioContextRef.current, 'recorder-processor');
            sourceRef.current.connect(workletNode);
            workletNode.connect(audioContextRef.current.destination);

            workletNode.port.onmessage = (e) => {
                if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
                    audioContextRef.current.resume(); // Ensure it's running
                }
                ...
                // Downsample to 16kHz
                const resampled = downsampleBuffer(inputData, sampleRate, 16000);
                
                // Convert to Int16
                const int16Data = floatTo16BitPCM(resampled);
                
                socket.emit('client_audio_chunk', int16Data.buffer);
            };
            ...
            if (err.name === 'NotAllowedError') errorMsg += "Permission denied. Please check site settings.";
            else if (err.name === 'NotFoundError') errorMsg += "No microphone found.";
            else errorMsg += err.message;
            
            setStatus('Mic Error');
            addMessage({ sender: 'System', text: errorMsg });
            alert(errorMsg);
        }
    };
    ...
    const stopMicVisualizer = () => {
        ...
        if (audioContextRef.current) {
             ...
             audioContextRef.current = null; // IMPORTANT: Reset to null so it gets recreated
        }
    };
    ...
    const togglePower = () => {
        if (isConnected) {
            ...
            const index = micDevices.findIndex(d => d.deviceId === selectedMicId);
            socket.emit('start_audio', { device_index: index >= 0 ? index : null, use_client_audio: true });
            setIsConnected(true);
    ...
    const handleMinimize = () => console.log("Minimize not supported in browser"); // ipcRenderer.send('window-minimize');
    const handleMaximize = () => console.log("Maximize not supported in browser"); // ipcRenderer.send('window-maximize');
    ...
    const handleCloseRequest = () => {
        ...
        const closeWindow = () => console.log("Close not supported in browser from renderer"); // ipcRenderer.send('window-close');
    ...
    return (
        <div className="h-screen w-screen bg-black text-cyan-100 font-mono overflow-hidden flex flex-col relative selection:bg-cyan-900 selection:text-white">
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
             ...
             {/* Global Audio Unlocker (Auto-resume on any click) */}
             <div style={{display: 'none'}} ref={el => {
                if (el && !el.hasUnlocker) {
                    el.hasUnlocker = true;
                    const unlockAudio = () => {
                        if (audioContextRef.current && audioContextRef.current.state === 'suspended') {
            ...
            <div style={{
                ...
                <div>MIC ID: {selectedMicId ? selectedMicId.substring(0, 15) + '...' : 'NONE'}</div>
                <div>MIC PERM: {micDevices.length > 0 ? 'ACCESSIBLE' : 'UNKNOWN/DENIED'}</div>
                <div>AUDIO CTX: {audioContextRef.current ? audioContextRef.current.state : 'NULL'}</div>
                <div>MUTED: {isMuted.toString()}</div>
                <div>MIC DATA: {micAudioData.reduce((a,b)=>a+b, 0) > 0 ? 'DETECTED' : 'SILENCE'}</div>
            </div>
        </div>
    );
}
