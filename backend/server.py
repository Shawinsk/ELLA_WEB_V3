...
async def start_audio(sid, data=None):
    global audio_loop, loop_task, client_input_queue
    ...
    if data:
        ...
        if 'use_client_audio' in data:
            use_client_audio = data['use_client_audio']
            
    print(f"Using input device: Name='{device_name}', Index={device_index}, ClientStreaming={use_client_audio}")
    ...
    # Setup Input Queue for Client Streaming
    input_queue = None
    if use_client_audio:
        input_queue = asyncio.Queue()
        client_input_queue = input_queue
        print("[SERVER] Initialized Client Audio Input Queue")
    ...
    # ... (other callbacks omitted, assuming they are same as before, I only need to construct AudioLoop correctly)
...
async def client_audio_chunk(sid, data):
    # data: {'data': [int16 array] or blob}
    global client_input_queue, audio_loop
    if client_input_queue and audio_loop:
        # Convert data to bytes if needed
        audio_data = data
        if isinstance(data, dict):
             audio_data = data.get('data') # In case wrapped
        
        # If it comes as list (Int array from JS), pack it
        if isinstance(audio_data, list):
             import struct
             # Assuming Int16 Little Endian
             try:
                audio_data = struct.pack(f"<{len(audio_data)}h", *audio_data)
             except Exception as e:
                print(f"Error packing audio: {e}")
                return
        
        # DEBUG: Print first few bytes of one in every 50 packets to verify stream
        # static counter hack
        if not hasattr(client_audio_chunk, "counter"): client_audio_chunk.counter = 0
        client_audio_chunk.counter += 1
        if client_audio_chunk.counter % 50 == 0:
            print(f"[AUDIO STREAM] Received chunk {client_audio_chunk.counter}, size: {len(audio_data)}")

        await client_input_queue.put(audio_data)

@sio.event
async def discover_kasa(sid):
    global SETTINGS
    print(f"Received discover_kasa request. SETTINGS type: {type(SETTINGS)}")
    try:
