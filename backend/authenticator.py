try:
    ...
    mp_python = None
    vision = None
    print("[AUTH] Warning: mediapipe not installed. Face auth disabled.")

try:
    import cv2
except ImportError:
    cv2 = None
    print("[AUTH] Warning: opencv-python not installed. Face auth disabled.")
import asyncio
import os
import base64
import numpy as np
...
class FaceAuthenticator:
    ...
    def __init__(self, reference_image_path="reference.jpg", on_status_change=None, on_frame=None):
        ...
        if not mp or not cv2:
            print("[AUTH] Dependencies missing. Authentication disabled.")
            return
    ...
    def _compare_landmarks(self, landmarks1, landmarks2, threshold=0.15):
        ...
        if landmarks1 is None:
            print("[AUTH] Debug: No reference landmarks. Bypassing auth check (Allow Any Face).")
            return True
    ...
    async def start_authentication_loop(self):
        ...
        if self.reference_landmarks is None:
             print("[AUTH] [WARN] No reference landmarks. Camera will run but AUTH WILL FAIL.")
             # return  <-- Allow camera to start for preview
    ...
    def _run_cv_loop(self, loop):
        def try_open_camera(index):
            ...
            # Use DirectShow on Windows for better compatibility, or valid default
            if os.name == 'nt':
